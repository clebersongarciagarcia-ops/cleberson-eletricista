...
export async function onRequest(context){
  ...
  if(!env.DB)return bad("D1 não está conectado.",503);
  ...
  try{
    if(request.method==="GET"&&path==="/bookings"){
      const {results}=await env.DB.prepare("SELECT * FROM bookings ORDER BY date ASC,time ASC").all();
      return json(results);
    }

    if(request.method==="POST"&&path==="/bookings"){
      const b=await body(request);
      const req=["service","date","time","name","phone"];

      if(req.some(k=>!String(b[k]||"").trim()))
        return bad("Campos obrigatórios ausentes.");

      const now=new Date().toISOString();

      const r=await env.DB.prepare(
        `INSERT INTO bookings(service,date,time,name,phone,address,details,value,status,created_at,updated_at)
         VALUES(?,?,?,?,?,?,?,?,?,?,?)`
      ).bind(
        b.service,
        b.date,
    ...
    if(match&&request.method==="PATCH"){
      ...
      if(!["Pendente","Confirmado","Concluído","Cancelado"].includes(b.status))
    ...
    if(request.method==="GET"&&path==="/clients"){
      ...
      const {results}=await env.DB.prepare(
        `SELECT name,phone,MAX(address) address,COUNT(*) total_bookings
         FROM bookings
...
if(request.method==="POST"&&path==="/budgets"){
  ...
  return json({budget:x},201);
    ...
    return bad("Erro interno: "+e.message,500);
  }
}
